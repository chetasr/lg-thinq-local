// LG Dishwasher (H11) handler
// Vendored from upstream PR anszom/rethink#139 (asata/feat/lg-dishwasher-h11), final revision.
// Local AABB parser: status packets are `32 ec` (with one or more `00 18` 24-byte status
// blocks), energy statistics are `32 3e`, control commands are `f0 26 ...`.
// Note (jmhufford's live deployment): the appliance does NOT stream continuously - it goes
// quiet for minutes, then bursts for about a minute (0x0A every 1-5s, 0x3E every 4-6s).
// The 0x0A type fails the AABB checksum by design (length byte reads 0xFF) - harmless.
    ...
    setProperty(prop: string, mqttValue: string) {
        if (prop === 'power') {
            ...
            if (!isNaN(val)) {
                ...
                this.publishProperty('target_extra_rinse', val)
            }
